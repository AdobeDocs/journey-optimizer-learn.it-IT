---
user-guide-title: 'Tutorial: configurare una sandbox di formazione'
user-guide-description: Configurare una sandbox per scopi di formazione
breadcrumb-title: 'Tutorial: configurare una sandbox di formazione'
breadcrumb-url: /docs/journey-optimizer-learn/configure-a-training-sandbox/introduction-and-prerequisites.html
index: false
source-git-commit: b91d6ccdb54213873b91b7ffa9d95d7cb5261ee8
workflow-type: tm+mt
source-wordcount: '0'
ht-degree: 0%

---


# Tutorial: configurare una sandbox di formazione {#configure-a-training-sandbox}

+ [Configurare una sandbox di formazione](/help/tutorial-configure-a-training-sandbox/introduction-and-prerequisites.md)
+ Passaggio 1: Impostare la struttura dei dati e importare dati di esempio {#set-up-data-structure-and-ingest-data}
   + [Configurazione della struttura dei dati e importazione dei dati di esempio tramite Postman](https://experienceleague.adobe.com/docs/platform-learn/tutorials/import-sample-data.html?lang=it)
   + [Configurare la struttura dati (manualmente)](/help/tutorial-configure-a-training-sandbox/manual-data-set-up.md)
   + [Importare dati di esempio (manualmente)](/help/tutorial-configure-a-training-sandbox/manual-data-ingestion.md)
+ [Passaggio 2: configurare gli eventi](/help/tutorial-configure-a-training-sandbox/configure-events.md)
+ [Tutorial](https://experienceleague.adobe.com/docs/journey-optimizer-learn/tutorials/overview.html?lang=it)
+ [Sfide](https://experienceleague.adobe.com/docs/journey-optimizer-learn/challenges/introduction-and-prerequisites.html?lang=it)
